...
def server(input, output, session):
    ...
    def season_selector():
        ...
        choices.update({str(row["season_id"]): row["season_name"] for _, row in relevant.iterrows()})
    ...
    def get_matches():
        if not input.competition_ids() or not input.season_ids():
            return None
        sel_c, sel_s = input.competition_ids(), input.season_ids()
        comps = get_competitions()
        comp_ids = comps["competition_id"].unique() if "all" in sel_c else [int(c) for c in sel_c]
        all_matches = []
        for c_id in comp_ids:
            s_ids = comps[comps["competition_id"] == c_id]["season_id"].unique() if "all" in sel_s else [int(s) for s in sel_s]
            for s_id in s_ids:
                try:
                    all_matches.append(sb.matches(competition_id=int(c_id), season_id=int(s_id)))
                except Exception:
                    continue
